	...
	doublereal *wa1, doublereal *wa2, doublereal *wa3, doublereal *wa4)
{
    ...
    extern /* Subroutine */ int qrfac_(integer *, integer *, doublereal *, 
	    ...
	    doublereal *, doublereal *, integer *), lmpar_(integer *, 
...
/* lva     info = 9  illegal division resulting in divergence. */

/* lva     info =10  square root of negative number. */
...
/* L100: */
	...
	if ((d__1 = fjac[j + j * fjac_dim1], abs(d__1)) <= dpmpar_(&c__2) * 
		abs(sum)) {
	    *info = 9;
	    return 0;
	}
...
/*        compute the norm of the scaled gradient. */
	...
	for (i__ = 1; i__ <= i__2; ++i__) {
	    if (abs(fnorm) <= dpmpar_(&c__2) * (d__1 = fjac[i__ + j * 
		    fjac_dim1] * qtf[i__], abs(d__1))) {
		*info = 9;
		return 0;
	    }
...
/* L140: */
	...
	if ((d__1 = wa2[l], abs(d__1)) <= dpmpar_(&c__2) * abs(sum)) {
	    *info = 9;
	    return 0;
	}
	if ((d__1 = sum / wa2[l], abs(d__1)) > gnorm) {
	    gnorm = (d__2 = sum / wa2[l], abs(d__2));
	}
...
/*           determine the levenberg-marquardt parameter. */
    ...
    lmpar_(n, &fjac[fjac_offset], ldfjac, &ipvt[1], &diag[1], &qtf[1], &delta,
	     &par, &wa1[1], &wa2[1], &wa3[1], &wa4[1], &info1);
    if (info1 > 8) {
	*info = info1;
	return 0;
    }
...
/*           compute the scaled actual reduction. */
    ...
    actred = -one;
    if (abs(fnorm) <= sqrt(dpmpar_(&c__2)) * abs(fnorm1)) {
	*info = 9;
	return 0;
    }
    if (p1 * fnorm1 < fnorm) {
...
/* L230: */
    ...
    if (abs(fnorm) <= dpmpar_(&c__2) * (d__1 = enorm_(n, &wa3[1]), abs(d__1)))
	     {
	*info = 9;
	return 0;
    }
	...
	*info = 10;
    ...
    if (abs(fnorm) <= dpmpar_(&c__2) * (d__1 = sqrt(par) * pnorm, abs(d__1))) 
	    {
	*info = 9;
	return 0;
    }
...
/*           reduction. */
    ...
    ratio = zero;
    if (abs(prered) <= dpmpar_(&c__2) * abs(actred)) {
	*info = 9;
	return 0;
    }
    if (prered != zero) {
...
/*           update the step bound. */
    ...
    if ((d__1 = dirder + p5 * actred, abs(d__1)) <= dpmpar_(&c__2) * (d__2 = 
	    p5 * dirder, abs(d__2))) {
	*info = 9;
	return 0;
    }
...
/* Computing MIN */
    ...
    if (abs(temp) <= dpmpar_(&c__2) * abs(par)) {
	*info = 9;
	return 0;
    }
    par /= temp;
	...
	doublereal *wa2, integer *info)
{
    ...
    static integer info1, i__, j, k, l;
...
/* lva   info is a fail code for division resulting in divergence */
...
/* L10: */
    ...
    for (k = 1; k <= i__1; ++k) {
	j = nsing - k + 1;
	if ((d__1 = r__[j + j * r_dim1], abs(d__1)) <= dpmpar_(&c__2) * (d__2 
		= wa1[j], abs(d__2))) {
	    *info = 9;
	    return 0;
	}
...
/*     the function. otherwise set this bound to zero. */
    ...
    for (j = 1; j <= i__1; ++j) {
	l = ipvt[j];
	if (abs(dxnorm) <= dpmpar_(&c__2) * (d__1 = wa2[l] * diag[l], abs(
		d__1))) {
	    *info = 9;
	    return 0;
	}
...
/* L110: */
    ...
    temp = enorm_(n, &wa1[1]);
    if ((d__1 = temp * temp * *delta, abs(d__1)) <= dpmpar_(&c__2) * abs(fp)) 
	    {
	*info = 9;
	return 0;
    }
...
/* L130: */
	...
	l = ipvt[j];
	if ((d__1 = diag[l], abs(d__1)) <= dpmpar_(&c__2) * abs(sum)) {
	    *info = 9;
	    return 0;
	}
	wa1[j] = sum / diag[l];
/* L140: */
    ...
    if (abs(*delta) <= dpmpar_(&c__2) * abs(gnorm)) {
	*info = 9;
	return 0;
    }
    paru = gnorm / *delta;
...
/*     set par to the closer endpoint. */
    ...
    *par = max(*par,parl);
    *par = min(*par,paru);
    if (abs(dxnorm) <= dpmpar_(&c__2) * abs(gnorm)) {
	*info = 9;
	return 0;
    }
    if (*par == zero) {
...
/*        compute the newton correction. */
    ...
    for (j = 1; j <= i__1; ++j) {
	l = ipvt[j];
	if (abs(dxnorm) <= dpmpar_(&c__2) * (d__1 = diag[l] * wa2[l], abs(
		d__1))) {
	    *info = 9;
	    return 0;
	}
...
/* L180: */
    ...
    i__1 = *n;
    for (j = 1; j <= i__1; ++j) {
	if ((d__1 = sdiag[j], abs(d__1)) <= dpmpar_(&c__2) * (d__2 = wa1[j], 
		abs(d__2))) {
	    *info = 9;
	    return 0;
	}
...
/* L210: */
    ...
    temp = enorm_(n, &wa1[1]);
    if ((d__1 = *delta * temp * temp, abs(d__1)) <= dpmpar_(&c__2) * abs(fp)) 
	    {
	*info = 9;
	return 0;
    }
...
/* Subroutine */ int qrfac_(integer *m, integer *n, doublereal *a, integer *
	lda, logical *pivot, integer *ipvt, integer *lipvt, doublereal *rdiag,
	 doublereal *acnorm, doublereal *wa, integer *info)
{
    /* Initialized data */
...
/*        j-th column of a to a multiple of the j-th unit vector. */
	...
	i__2 = *m;
	for (i__ = j; i__ <= i__2; ++i__) {
	    if (abs(ajnorm) <= dpmpar_(&c__2) * (d__1 = a[i__ + j * a_dim1], 
		    abs(d__1))) {
		*info = 9;
		return 0;
	    }
...
/* L60: */
	    ...
	    if ((d__1 = a[j + j * a_dim1], abs(d__1)) <= dpmpar_(&c__2) * abs(
		    sum)) {
		*info = 9;
		return 0;
	    }
...
/* L70: */
	    ...
	    if ((d__1 = rdiag[k], abs(d__1)) <= dpmpar_(&c__2) * (d__2 = a[j 
		    + k * a_dim1], abs(d__2))) {
		*info = 9;
		return 0;
	    }
...
/* Computing 2nd power */
	    ...
	    if ((d__1 = wa[k], abs(d__1)) <= sqrt(dpmpar_(&c__2)) * (d__2 = 
		    rdiag[k], abs(d__2))) {
		*info = 9;
		return 0;
	    }
...
/* Subroutine */ int qrsolv_(integer *n, doublereal *r__, integer *ldr, 
	...
	doublereal *sdiag, doublereal *wa, integer *info)
{
    /* Initialized data */
    ...
    static doublereal tan__, cos__, sin__, sum;
...
/* Computing 2nd power */
	    d__1 = cotan;
	    if (p25 + p25 * (d__1 * d__1) < 0.) {
		*info = 10;
		return 0;
	    }
/* Computing 2nd power */
	    d__2 = cotan;
	    if ((d__1 = sqrt(p25 + p25 * (d__2 * d__2)), abs(d__1)) <= 
		    dpmpar_(&c__2)) {
		*info = 9;
		return 0;
	    }
/* Computing 2nd power */
	    ...
	    goto L50;
L40:
	    if ((d__1 = r__[k + k * r_dim1], abs(d__1)) <= dpmpar_(&c__2) * (
		    d__2 = sdiag[k], abs(d__2))) {
		*info = 9;
		return 0;
	    }
...
L130:
	if ((d__1 = sdiag[j], abs(d__1)) <= dpmpar_(&c__2) * (d__2 = wa[j] - 
		sum, abs(d__2))) {
	    *info = 9;
	    return 0;
	}
